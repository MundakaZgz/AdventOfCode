...
function simulateGuardPatrol(guardPosition, map) {
  ...
  const visitedCellsWithDirection = [];
    ...
    switch (currentDirection) {
      case 0:
        if (
          ...
          map[guardPosition.y - 1][guardPosition.x] !== "O"
        ) {
          guardPosition.y--;
          map[guardPosition.y][guardPosition.x] = "X";
          if (
            visitedCellsWithDirection.includes(
              `${guardPosition.y}-${guardPosition.x}-${currentDirection}`,
            )
          ) {
            return true;
          }
          visitedCellsWithDirection.push(
            `${guardPosition.y}-${guardPosition.x}-${currentDirection}`,
          );
        } else {
          currentDirection = 1;
      ...
      case 1:
        if (
          ...
          map[guardPosition.y][guardPosition.x + 1] !== "O"
        ) {
          guardPosition.x++;
          map[guardPosition.y][guardPosition.x] = "X";
          if (
