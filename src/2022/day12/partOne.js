...
module.exports = function () {
  ...
  const generateTree = (node, matrix) => {
    ...
    DIRECTIONS.forEach((direction) => {
      ...
      if (newPosition.row >= 0 && newPosition.column < MAX_ROWS && newPosition.column >= 0 && newPosition.column < MAX_COLUMNS) {
        const newValue = matrix[newPosition.row][newPosition.column];
        if (newValue == node.value + 1) {
          const newNode = new Node(newPosition.row, newPosition.column, newValue);
  ...
  const main = () => {
    ...
    const root = new Node(startingPoint.row, startingPoint.column, 'a'.charCodeAt(0));
