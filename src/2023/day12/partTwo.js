...
module.exports = function () {
  ...
  function findCombinations(configuration, records) {
    ...
    return (cache[line] ??=
      (configuration[0] != "#"
        ? findCombinations(trimStart(configuration.slice(1)), records)
        : 0) +
      (damagedOrUnknown && configuration[records[0]] != "#"
        ? findCombinations(
            trimStart(configuration.slice(records[0] + 1)),
            records.slice(1),
          )
        : 0));
  }
  const main = () => {
    ...
    for (const line of data) {
      ...
      result += findCombinations(
        Array(5).fill(configuration).join("?"),
        Array(5).fill(records).flat(),
      );
    }
