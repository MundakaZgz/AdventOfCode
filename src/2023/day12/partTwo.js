...
module.exports = function () {
  ...
  const main = () => {
    ...
    for(let i=0; i<configurationsAndRecords.configurations.length; i++) {
      ...
      let arrangement = getArrangements(configuration, validationFunction);
      let combinations = [...arrangement];
      for(let times = 0; times < 4; times++) {
          let newCombinations = new Set();
          for(let i=0; i<combinations.length; i++) {
              for(let j=0; j<arrangement.length; j++) {
                  newCombinations.add(combinations[i] + arrangement[j]);
              }
          }
          combinations = [...newCombinations].filter((x) => !/#{4}/g.test(x));
      }

      arrangements.push(combinations);
    }
