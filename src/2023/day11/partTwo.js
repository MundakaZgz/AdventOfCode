...
module.exports = function () {
  ...
  function pairGalaxies(galaxies) {
    ...
    for (let i = 0; i < galaxies.length; i++) {
      for (let j = i + 1; j < galaxies.length; j++) {
        ...
        const pairId = galaxies[i].id < galaxies[j].id ? `${galaxies[i].id}-${galaxies[j].id}` : `${galaxies[j].id}-${galaxies[i].id}`;
  ...
  const main = () => {
    ...
    const distances = Object.values(pairedGalaxies).map((pair) => getDistance(pair.galaxy1, pair.galaxy2, expansionAreas));
